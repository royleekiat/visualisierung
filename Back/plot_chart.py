...
def plotChart(choice):

    print(choice)
    #Do something with this choice
    #change savefig() to save as an image. priority being svg but png/jpg is fine too
    ...
    if method=='BITES' or method=='ITES':
        ...
        if compare_against_ATE:
            ...
            analyse_randomized_test_set(pred_ite, Y_test, event_test, treatment_test,C_index=C_index,method_name=method, save_path='./charts/RGBSG_'+method+'_baseline.pdf',new_figure=False,annotate=True)
        else:
            analyse_randomized_test_set(pred_ite, Y_test, event_test, treatment_test, C_index=C_index,
                                        method_name=method,
                                        save_path='./charts/RGBSG_' + method + '.pdf')
        ...
        #sns.set(font_scale=1)
        analyse_randomized_test_set(pred_ite, Y_test, event_test, treatment_test, C_index=C_index, method_name='T-DeepSurv',
    ...
    if do_SHAP and method=='BITES':
        ...
        fig.tight_layout() 
        plt.savefig('./charts/SHAP_RGBSG_BITES.pdf', format='pdf')